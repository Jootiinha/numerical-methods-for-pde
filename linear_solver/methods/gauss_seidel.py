"""
Método iterativo de Gauss-Seidel para resolução de sistemas lineares.
"""

from typing import Tuple, Optional, Dict, Any
import numpy as np
from ..base import LinearSolver


class GaussSeidelSolver(LinearSolver):
    """
    Método iterativo de Gauss-Seidel para resolução de sistemas lineares.
    
    Similar ao Jacobi, mas utiliza os valores já atualizados na mesma iteração,
    geralmente convergindo mais rapidamente.
    
    Fórmula iterativa: x_i^(k+1) = (b_i - Σ(A_ij * x_j^(k+1)) - Σ(A_ij * x_j^(k))) / A_ii
    onde a primeira soma é sobre j < i e a segunda sobre j > i.
    """
    
    def get_method_name(self) -> str:
        return "Gauss-Seidel"
    
    def solve(self, A: np.ndarray, b: np.ndarray, 
              x0: Optional[np.ndarray] = None) -> Tuple[np.ndarray, Dict[str, Any]]:
        """
        Resolve o sistema usando o método de Gauss-Seidel.
        
        Args:
            A: Matriz de coeficientes (deve ter diagonal não-nula)
            b: Vetor de termos independentes
            x0: Aproximação inicial (padrão: vetor nulo)
            
        Returns:
            Tupla (solução, informações_convergência)
        """
        self._validate_inputs(A, b)
        
        # Verificar se a diagonal principal é não-nula
        diagonal = np.diag(A)
        if np.any(np.abs(diagonal) < 1e-14):
            raise ValueError("Matriz A deve ter diagonal principal não-nula")
        
        n = A.shape[0]
        x = self._get_initial_guess(A, x0)
        
        self.convergence_history = []
        
        for iteration in range(self.max_iterations):
            x_old = x.copy()
            
            # Aplicar fórmula iterativa de Gauss-Seidel
            for i in range(n):
                # Soma dos termos já atualizados (j < i)
                sum_lower = sum(A[i, j] * x[j] for j in range(i))
                # Soma dos termos ainda não atualizados (j > i)  
                sum_upper = sum(A[i, j] * x_old[j] for j in range(i + 1, n))
                
                x[i] = (b[i] - sum_lower - sum_upper) / A[i, i]
            
            # Calcular erro e verificar convergência
            error = np.linalg.norm(x - x_old, ord=np.inf)
            self.convergence_history.append(error)
            
            if self._check_convergence(x, x_old):
                info = {
                    'converged': True,
                    'iterations': iteration + 1,
                    'final_error': error,
                    'method': self.get_method_name(),
                    'convergence_history': self.convergence_history.copy()
                }
                return x.copy(), info
        
        # Não convergiu
        info = {
            'converged': False,
            'iterations': self.max_iterations,
            'final_error': self.convergence_history[-1],
            'method': self.get_method_name(),
            'convergence_history': self.convergence_history.copy()
        }
        return x.copy(), info
