#!/usr/bin/env python3
"""
Exemplo de resolução do sistema não linear específico.
...
Transformando para a forma F(x) = 0:
...
# Importar matplotlib apenas se disponível
try:
...
def main():
    ...
    # Executar com diferentes tolerâncias
    tolerances = [1e-4, 1e-6]
    
    for tol in tolerances:
        print(f"\\n{'='*70}")
        print(f"EXECUTANDO COM TOLERÂNCIA: {tol:.0e}")
        print(f"{'='*70}")
        
        results = example.run_all_methods(tolerance=tol, max_iterations=1000)
        ...
        for method_name, method_results in results.items():
            converged_results = [r for r in method_results if r.get('converged', False)]
            if converged_results:
                best = min(converged_results, key=lambda r: r['function_norm'])
                best_solutions[method_name] = best
        
        print(f"\\n🏆 MELHORES SOLUÇÕES (Tolerância {tol:.0e}):")
        print("-" * 50)
        for method_name, result in best_solutions.items():
            x, y, z = result['solution']
            print(f"{method_name:10}: x={x:.6f}, y={y:.6f}, z={z:.6f}, ||F||={result['function_norm']:.2e}, iter={result['info']['iterations']}")
