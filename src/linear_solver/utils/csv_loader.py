...
class CSVMatrixLoader:
    ...
    @staticmethod
    def load_augmented_matrix(filepath: Union[str, Path], 
                            delimiter: str = ',',
                            skip_header: bool = False) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        try:
            ...
            if n_cols < 2:
                raise ValueError("Matriz deve ter pelo menos 2 colunas (coeficientes + termo independente)")
            ...
            if A.shape[0] != A.shape[1]:
                raise ValueError(f"Matriz de coeficientes deve ser quadrada. "
    ...
    @staticmethod
    def load_separate_files(matrix_filepath: Union[str, Path], 
        ...
        """
        # Carregar matriz A
        A = CSVMatrixLoader._load_matrix_from_csv(matrix_filepath, delimiter, skip_header)
        
        # Carregar vetor b
        b = CSVMatrixLoader._load_vector_from_csv(vector_filepath, delimiter, skip_header)
        
        # Verificar compatibilidade de dimensões
        if A.shape[0] != b.shape[0]:
            raise ValueError(f"Dimensões incompatíveis: matriz A ({A.shape[0]}x{A.shape[1]}) "
                           f"e vetor b ({b.shape[0]})")
        
        # Verificar se A é quadrada
        if A.shape[0] != A.shape[1]:
            raise ValueError(f"Matriz A deve ser quadrada. Encontrada: {A.shape[0]}x{A.shape[1]}")
        
        return A, b
    
    @staticmethod
    def _load_matrix_from_csv(filepath: Union[str, Path], 
                            delimiter: str = ',',
                            skip_header: bool = False) -> np.ndarray:
    ...
    @staticmethod 
    def _load_vector_from_csv(filepath: Union[str, Path],
                            delimiter: str = ',', 
                            skip_header: bool = False) -> np.ndarray:
    ...
    @staticmethod
    def save_augmented_matrix(A: np.ndarray, b: np.ndarray, 
